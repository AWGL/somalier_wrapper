...
def parse_arguments():
	...
	argument_parser.add_argument(
		"--sample_info_csv",
		help="requires following columns: sample_id, cram_path and (optional) patient_identifier",
		required=True,
	)
	argument_parser.add_argument(
	...
	argument_parser.add_argument(
		"--patient_identifier",
		help="Column specifying unique patient identifier (optional)",
		required=False,
	)
	argument_parser.add_argument(
		"--somalier_1K_directory",
		help="Path to directory containing .somalier files for 1K genomes",
		required=False,
	)
	argument_parser.add_argument(
		"--somalier_1K_labels",
		help="Path to labels file for 1K genomes samples",
		required=False,
	)
		...
		for row in reader:
			...
			try:
				if os.path.exists(f"{output_dir}/{sample_id}.somalier"):
					logging.info(
						f"Extraction successful for sample: {sample_id} (using previous extraction)"
					)
				else:
...
def somalier_relatedness(file, prefix):
	...
	somalier_files = glob.glob(
		"somalier_output/temp/*.somalier"
	)  ## <- update so that only looks at specified samples
	if not somalier_files:
		logging.warning(
			"No .somalier files found in 'somalier_output/temp/' directory."
		)
		return
...
def somalier_relatedness_expected(file, identifier, prefix):
	...
	grouped = (
		df.groupby(identifier)["sample_id"].apply(lambda x: ",".join(x)).reset_index()
	)
	...
	try:
		subprocess.run(cmd, shell=True, check=True)
		logging.info(
			f"Relatedness analysis completed with groups file: somalier_output/expected_relationships.csv"
		)
	except subprocess.CalledProcessError as e:
		logging.error("Error occurred during somalier relate")
		logging.error(str(e))
...
def main():
	...
	if args.patient_identifier:
		somalier_relatedness_expected(
			args.sample_info_csv, args.patient_identifier, args.prefix
		)
	else:
